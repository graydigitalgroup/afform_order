...
class OrderModifyValidateEvent extends Event {
  ...
  /**
   * Structured detail set by a subscriber that vetoes a refund-producing edit
   * but wants to convey what was intended, so a UI can offer to create a proper
   * refund request from it. NULL when not a refund-routing veto.
   *
   * Shape (subscriber-defined, but conventionally):
   *   [
   *     'reason' => 'refund_required',
   *     'contribution_id' => int,
   *     'net_delta' => float,        // negative
   *     'line_items_to_remove' => array,
   *     'line_items_to_add' => array,
   *   ]
   *
   * @var array|null
   */
  private ?array $refundRequiredDetail = NULL;
  ...
  /**
   * Mark this veto as a "refund required" routing case and attach the intended
   * change, so a caller/UI can offer to create a refund request from it. This
   * does NOT by itself stop the modify - call addError() too (or use
   * vetoForRefund() which does both).
   *
   * @param array $detail
   * @return void
   */
  public function setRefundRequiredDetail(array $detail): void {
    $this->refundRequiredDetail = $detail;
  }

  /**
   * @return array|null
   */
  public function getRefundRequiredDetail(): ?array {
    return $this->refundRequiredDetail;
  }

  /**
   * True when a subscriber vetoed specifically because a refund is required
   * (vs. any other validation failure). Lets a caller distinguish "this needs
   * to go through the refund flow" from a generic error.
   *
   * @return bool
   */
  public function isRefundRequiredVeto(): bool {
    return $this->refundRequiredDetail !== NULL;
  }

  /**
   * Convenience: veto the edit AND record the refund-required detail in one
   * call. The error message stops the modify; the detail lets a UI offer to
   * create a refund request from the intended change.
   *
   * @param string $errorMsg
   * @param array $detail
   * @return void
   */
  public function vetoForRefund(string $errorMsg, array $detail): void {
    $this->addError($errorMsg);
    $this->setRefundRequiredDetail($detail);
  }
